...
int main(int argc, char *argv[]) {
	...
	while (true) {
		...
		if (!strncmp(command, "cd ", 3)) {
			...
			if (!changeDirectory(&command[3]))
				printf(">baash - cd: inexistent directory [%s]\n", &command[3]);
			continue;
		}
...
void showScreen(int screen) {
	...
	if (screen == SCREEN_START)
		printf(">baash started\n");
	if (screen == SCREEN_FINISH)
		printf(">baash terminated.\n");
}
...
void execute(char* instruction, int INPUT, int OUTPUT) {
	...
	if (!hasArguments)
		strcpy(result, instruction); // case [cmd]
	...
	if (execv(result, arguments)) { // execv returns only if there is an error
		...
		sprintf(error, "process %d returned error %d when executing %s",
				getpid(), errno, result); // should never run
		perror(error);
		exit(EXIT_FAILURE);
	}
}
...
void redirect(int FD_input, int FD_output) {
	if (verbose)
		printf(
				...
				pName, FD_input, FD_output, pipeA[0], pipeA[1], pipeB[0],
				pipeB[1]);
	if (dup2(FD_input, fileno(stdin)) < 0
			|| dup2(FD_output, fileno(stdout)) < 0) {
